public class T4 extends Thread {
  private int z4_1; // max(Zh)
  private int z_4; // z
  private int[] B_4; // B
  private int[] X_4; // X
  private int e_4; // e

  @Override
  public void run() {
    System.out.println("4: Старт T4");

    System.out.println("4: Ввід Вектору Z");
    Data.Z = Data.insertVecWithNum(1);

    System.out.println("4: Ввід Вектору X");
    Data.X = Data.insertVecWithNum(1);

    System.out.println("4: Ввід Вектору B");
    Data.B = Data.insertVecWithNum(1);

    System.out.println("4: Ввід e");
    Data.e = 1;

    System.out.println("4: T4 звільнює дозвіл для інших семафорів");
    Data.SemInput_T4.release(3);

    try {
      System.out.println("4: T4 чекає на дозвіл T1 T2 про введення даних");
      Data.SemInput_T1.acquire();
      Data.SemInput_T2.acquire();
      System.out.println("4: T4 отримує дозвіл T1 T2");

      z4_1 = Data.maxSubArr(Data.Z, Data.H * 3, Data.N);
      Data.A1.set(Data.setMax(Data.A1, z4_1));

      System.out.println("4: T4 звільнює дозвіл для інших семафорів");
      Data.SemEndZ_T4.release(1);

      System.out.println("4: T4 чекає на EndZ_T[1, 2, 3]");
      Data.SemEndZ_T1.acquire();
      System.out.println("4: T4 отримало EndZ_T[1, 2, 3]");

      z_4 = Data.A1.get();

      synchronized (Data.CS1) {
      B_4 = Data.B;
      }

      Data.B1.lock();
      try {
          e_4 = Data.e;
      } finally {
          Data.B1.unlock();
      }

      int[] mulMtxVec1 = Data.mulMtxAndVec(Data.MV, Data.B);
      int[][] mulMtxVec2 = Data.mulSubMtx(Data.MM, Data.MC, Data.H * 3, Data.N);
      int[] part1 = Data.mulVecAndScalar(mulMtxVec1, Data.A1.get());
      int[] part2 = Data.mulMtxAndVec(mulMtxVec2, Data.X);
      int[] part3 = Data.mulVecAndScalar(part2, Data.e);
      int[] R_1 = Data.sumVecs(part1, part3);
      Data.saveToSubVector(R_1, Data.R, Data.H * 3, Data.N);

      Data.SemEnd_T4.release(1);

      System.out.println("4: T4 stopped");
    } catch (InterruptedException e) {
      throw new RuntimeException(e);
    }
  }
}

